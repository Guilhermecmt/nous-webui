# -*- coding: utf-8 -*-
"""
Instala/atualiza a funcao 'Nous Files' DIRETO no banco do Open WebUI, como
Filter GLOBAL e ATIVO. Sem credenciais, sem navegador, sem painel de admin.

Idempotente: pode rodar a cada inicializacao do Nous. Re-sincroniza o codigo de
nous_files.py (o arquivo em disco e' a fonte da verdade) e garante global+ativo.

USO:
    python files/register_files.py [--data-dir <DATA_DIR>] [--filter <arquivo>]
"""
import os
import sys
import time
...
FUNC_ID = "nous_files"
FUNC_NAME = "Nous Files"
DESC = ("Acesso aos seus arquivos locais: busca hibrida (semantica + palavra-chave) "
        "na sua pasta e injeta os trechos relevantes no chat. 100% local.")
...
def main():
    ...
    ap.add_argument("--filter", default=os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "nous_files.py"))
    a = ap.parse_args()
    ...
    if not db or not os.path.isfile(db):
        print("webui.db ainda nao existe (servidor nao iniciou). "
              "O Nous Files sera registrado na proxima inicializacao.")
        return
    ...
    try:
        ...
        cx.commit()
        print(f"Nous Files {action}: Filter global e ativo (id='{FUNC_ID}').")
    finally:
        cx.close()
